var BigInteger = require("./biginteger.js"),
    Sjcl       = require("../node_modules/sjcl-full/sjcl"),
    Hash       = Sjcl.hash,
...
module.exports = (function () {
   ...
   */
  srp.generateVerifier = function (params,salt,I,P) {
   /* note: semi-standardizing SRP and trying to bring it in line with the RFCs
    * this means that the order of exchange isn't going to be as dense as it 
    * possibly could be, since we need to allow for --- e.g., --- a SRP client/
    * server on the other end that we didn't pass across.
    *
    ...
    */
    var H = params.hash 
    var x = H(salt + H(identity + ":" + password));
   ...
   * - SRP parameters (see _defaults and paramsFromOptions below)
   */
  srp.Client = function (params,options) {
    var self = this;
  ...
  var paramsFromOptions = function (options) {
    ...
    if (typeof options=="string" && Params[options])
      return Params[options]
    var ret = srp._defaults||{};
